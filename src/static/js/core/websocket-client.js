...
export class MultimodalLiveClient extends EventEmitter {
	...
	this.url =
        url ||
	`wss://gemini.gemini2.us.kg/ws/google.ai.generativelanguage.v1alpha.GenerativeService.BidiGenerateContent?key=${CONFIG.API.KEY}`;
	this.ws = null;
	this.config = null;
	...
	sendRealtimeInput(chunks) {
		...
		const message =
			hasAudio && hasVideo
				...
				: "unknown";
		Logger.debug(
			`Sending realtime input: ${message} (${Math.round(totalSize / 1024)}KB)`
		);
